...
def print_info(attrs: list, tab_width=4, sort_by='name', sort_order='ascending'):
    ...
    # {plugin_object: {attr: value, ...}, ...}
    plugin_attrs = {plugin: {attr: get_plugin_attr(plugin, attr) for attr in attrs} for plugin in plugins}
    ...
    for plugin in plugins:  # print values
        line = "\t".join(["{attr:{str_len}}".format(attr=plugin_attrs[plugin][attr], str_len=attr_lens[attr])
                         for attr in attrs]).replace("\t", " "*tab_width)
        print(line)
